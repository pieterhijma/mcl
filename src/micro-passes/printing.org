...
(defun slot-name->string (slot-name)
  (kebab:to-camel-case (format nil "~a" slot-name)))

(defun slot-names (object)
  (mapcar #'slot-name->string (slot-names-children object)))

(defun print-slots (ast class-name)
  (emit-list (mapcar #'(lambda (slot-name slot-value)
			 (emit "\"~a\": ~a" slot-name (print-ast slot-value)))
		     (cons "class" (slot-names ast))
		     (cons class-name (slot-values ast)))
	     :separator ","
	     :nr-lines 1
	     ))

(defgeneric print-ast (ast))

(defmethod print-ast ((s string))
  (format nil "~s" s))

(defmethod print-ast ((n number))
  (format nil "~a" n))

(defmethod print-ast ((s (eql t)))
  "true")

(defmethod print-ast ((s (eql nil)))
  "false")

(defmethod print-ast ((l list))
  (if l
      (emit "["
	    "  ~a" (emit-list l :separator "," :nr-lines 1 :function #'print-ast)
	    "]")
      (emit "[]")))

(defmethod print-ast ((ast ast))
  (emit "{"
	"  ~a" (print-slots ast (kebab:to-camel-case (symbol-name (sb-mop:class-name (class-of ast)))))
	"}"))
